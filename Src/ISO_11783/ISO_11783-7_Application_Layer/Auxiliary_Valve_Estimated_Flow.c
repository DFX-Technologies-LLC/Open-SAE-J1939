...
{
    return SAE_J1939_Send_Request(j1939,
                                  DA,
                                  PGN_AUXILIARY_VALVE_ESTIMATED_FLOW_0
                                      + valve_number); /* valve_number can be 0 to 15 */
}
...
{
    uint32_t ID = (0x0CFE << 16) | ((0x10 + valve_number) << 8)
        | j1939->information_this_ECU.this_ECU_address;
    uint8_t data[8];
    data[0]
        = j1939->this_auxiliary_valve_estimated_flow[valve_number].extend_estimated_flow_standard;
    data[1]
        = j1939->this_auxiliary_valve_estimated_flow[valve_number].retract_estimated_flow_standard;
    data[2] = (j1939->this_auxiliary_valve_estimated_flow[valve_number].fail_safe_mode << 6)
        | (0b11 << 4)
        | j1939->this_auxiliary_valve_estimated_flow[valve_number]
              .valve_state; /* Bit 5 and 6 are reserved */
...
void ISO_11783_Read_Response_Request_Auxiliary_Estimated_Flow(J1939_t* j1939,
                                                              ...
                                                              uint8_t data[])
{
    j1939->from_other_ecu_auxiliary_valve_estimated_flow[valve_number]
        .extend_estimated_flow_standard
        = data[0];
    j1939->from_other_ecu_auxiliary_valve_estimated_flow[valve_number]
        .retract_estimated_flow_standard
        = data[1];
    j1939->from_other_ecu_auxiliary_valve_estimated_flow[valve_number].fail_safe_mode
        = data[2] >> 6;
    j1939->from_other_ecu_auxiliary_valve_estimated_flow[valve_number].valve_state
        = data[2] & 0b00001111;
