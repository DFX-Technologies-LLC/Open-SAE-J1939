...
{
    uint32_t ID = (0x0CFF << 16) | ((0x20 + valve_number) << 8)
    ...
    data[1] = j1939->this_auxiliary_valve_measured_position[valve_number].measured_position_percent
        >> 8;
    data[2] = 0b11110000 | j1939->this_auxiliary_valve_measured_position[valve_number].valve_state;
    data[3]
        = j1939->this_auxiliary_valve_measured_position[valve_number].measured_position_micrometer;
    data[4]
        = j1939->this_auxiliary_valve_measured_position[valve_number].measured_position_micrometer
        >> 8;
...
void ISO_11783_Read_Response_Request_Auxiliary_Valve_Measured_Position(J1939_t* j1939,
                                                                       ...
                                                                       uint8_t data[])
{
    j1939->from_other_ecu_auxiliary_valve_measured_position[valve_number].measured_position_percent
        = (data[1] << 8) | data[0];
    j1939->from_other_ecu_auxiliary_valve_measured_position[valve_number].valve_state
        = 0b00001111 & data[2];
    j1939->from_other_ecu_auxiliary_valve_measured_position[valve_number]
        .measured_position_micrometer
        = (data[4] << 8) | data[3];
