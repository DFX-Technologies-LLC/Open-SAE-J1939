...
/* Load our ECU parameters into J1939 structure. Very useful if you want your ECU remember its NAME
 * + address + identifications at startup. */
bool Open_SAE_J1939_Startup_ECU(J1939_t* j1939)
{
#if ECU_DATA_STORING_ENABLED
    ...
    memcpy(&j1939->information_this_ECU,
           (Information_this_ECU*)ECU_information_data,
           ECU_information_length);
#endif
    /* If we are going to send and receive the ECU identification and component identification, we
     * need to specify the size of them */
    j1939->information_this_ECU.this_identifications.ecu_identification.length_of_each_field
        = MAX_IDENTIFICATION;
    j1939->information_this_ECU.this_identifications.component_identification.length_of_each_field
        = MAX_IDENTIFICATION;
