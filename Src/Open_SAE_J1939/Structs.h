...
struct Acknowledgement
{
    ...
    uint8_t group_function_value; /* The function code that specify cause of the control byte e.g
                                     time out or aborted */
...
struct TP_CM
{
    ...
    /* RTS */
    uint16_t total_message_size_being_transmitted; /* Total bytes our complete message includes - 9
                                                      to 1785 */
    uint8_t number_of_packages_beging_transmitted; /* How many times we are going to send packages
                                                      via TP_DT - 2 to 224 because 1785/8 is 224
                                                      rounded up */

    /* CTS */
    uint8_t total_number_of_packages_transmitted; /* Total packages we have received */
    uint8_t next_packet_number_transmitted; /* Next packet number we want to have from the
                                               transmitter */
...
struct TP_DT
{
    uint8_t sequence_number; /* When this sequence number is the same as number_of_packages from
                                TP_CM, then we have our complete message */
    uint8_t data[MAX_TP_DT]; /* This is the collected data we are going to send. Also we are using
                                this as a filler */
    uint8_t from_ecu_address; /* From which ECU came this message */
};
...
struct Name
{
    ...
    uint8_t arbitrary_address_capable; /* Specify if the ECU have right to change address if
                                          addresses conflicts - 0 to 1 */
...
struct DM1
{
    ...
    uint8_t SPN_conversion_method
        [MAX_DM_FIELD]; /* If SPN_conversion_method = 1 that means Diagnostics Trouble Code are
                           aligned using a newer conversion method. If SPN_conversion_method = 0
                           means one of the three Diagnostics Trouble Code conversion methods is
                           used and ECU manufacture shall know which of the three methods is used */
    uint8_t occurrence_count[MAX_DM_FIELD]; /* This tells how many times failure has occurred. Every
                                               time fault goes from inactive to active, the
                                               occurence_count is incremented by 1. If fault becomes
                                               active for more than 126 times the occurence_count
                                               remains 126 */
...
{
    uint16_t
        number_of_allowed_bytes; /* How many bytes we are allowed to write or read to - 0 to 255 */
    uint8_t status; /* Status of the response */
    uint32_t EDC_parameter; /* Status code */
    uint8_t EDCP_extention; /* Describe how we should interpret the EDC parameter as a status code
                               or error code */
...
struct DM
{
    ...
    struct DM1
        dm1; /* dm1 can only hold 1 error message at the time, but we know how many errors exists */
    ...
    struct DM16 dm16; /* dm16 is the binary data transfer after DM15 memory response (if it was
                         proceeded) */
    /* Add more DM here */
};
...
/* This struct is used for save information and load information from hard drive/SD-card/flash etc.
 * due to the large size of J1939 */
typedef struct
{
...
typedef struct
{
    ...
    bool
        ID_and_data_is_updated; /* This is a flag that going to be set to true for every time ID and
                                   data updates - Very useful in higher applications such as C++ */

    /* Store addresses of ECU */
    uint8_t number_of_other_ECU; /* How many other ECU are connected */
    uint8_t
        number_of_cannot_claim_address; /* How many ECU addresses could not claim their address */
    uint8_t other_ECU_address[255]; /* Store other ECU addresses here. Address 0xFF is the broad
                                       cast address, not an ECU address */
