...
void SAE_J1939_Read_Transport_Protocol_Data_Transfer(J1939_t* j1939, uint8_t SA, uint8_t data[])
{
    ...
    for (i = 1; i < 8; i++) {
        j1939->from_other_ecu_tp_dt.data[index * 7 + i - 1]
            = data[i]; /* For every package, we send 7 bytes of data where the first byte data[0] is
                          the sequence number */
    }
    /* Check if we have completed our message - Return = Not completed */
    if (j1939->from_other_ecu_tp_cm.number_of_packages_beging_transmitted
            != j1939->from_other_ecu_tp_dt.sequence_number
        || j1939->from_other_ecu_tp_cm.number_of_packages_beging_transmitted == 0) {
    ...
    if (j1939->from_other_ecu_tp_cm.control_byte == CONTROL_BYTE_TP_CM_RTS) {
        ...
        j1939->this_ecu_tp_cm.total_number_of_bytes_received
            = j1939->from_other_ecu_tp_cm.total_message_size_being_transmitted;
        j1939->this_ecu_tp_cm.total_number_of_packages_received
            = j1939->from_other_ecu_tp_dt.sequence_number;
        SAE_J1939_Send_Transport_Protocol_Connection_Management(j1939, SA);
    }
...
ENUM_J1939_STATUS_CODES SAE_J1939_Send_Transport_Protocol_Data_Transfer(J1939_t* j1939, uint8_t DA)
{
    ...
    switch (j1939->from_other_ecu_tp_cm.control_byte) {
            ...
            for (i = 1; i <= j1939->this_ecu_tp_cm.number_of_packages_beging_transmitted; i++) {
                ...
                for (j = 0; j < 7; j++) {
                    if (bytes_sent < j1939->this_ecu_tp_cm.total_message_size_being_transmitted) {
                        package[j + 1] = j1939->this_ecu_tp_dt
                                             .data[bytes_sent++]; /* Data that we have collected */
                    } else {
        ...
        case CONTROL_BYTE_TP_CM_CTS:
            package[0] = j1939->from_other_ecu_tp_cm.next_packet_number_transmitted
                + 1; /* Next number of package */
