 ...
 * PGN: 0x00FECA (65226)
 */
ENUM_J1939_STATUS_CODES SAE_J1939_Send_Request_DM1(J1939_t* j1939, uint8_t DA)
{
 ...
 * PGN: 0x00FECA (65226)
 */
ENUM_J1939_STATUS_CODES SAE_J1939_Response_Request_DM1(J1939_t* j1939, uint8_t DA)
{
    if (j1939->this_dm.errors_dm1_active < 2) {
        uint32_t ID = (0x18FECA << 8) | j1939->information_this_ECU.this_ECU_address;
        uint8_t data[8];
        data[0] = (j1939->this_dm.dm1.SAE_lamp_status_malfunction_indicator << 6)
            | (j1939->this_dm.dm1.SAE_lamp_status_red_stop << 4)
            | (j1939->this_dm.dm1.SAE_lamp_status_amber_warning << 2)
            | (j1939->this_dm.dm1.SAE_lamp_status_protect_lamp);
        data[1] = (j1939->this_dm.dm1.SAE_flash_lamp_malfunction_indicator << 6)
            | (j1939->this_dm.dm1.SAE_flash_lamp_red_stop << 4)
            | (j1939->this_dm.dm1.SAE_flash_lamp_amber_warning << 2)
            | (j1939->this_dm.dm1.SAE_flash_lamp_protect_lamp);
        ...
        data[5] = (j1939->this_dm.dm1.SPN_conversion_method[0] << 7)
            | j1939->this_dm.dm1.occurrence_count[0];
        data[6] = 0xFF; /* Reserved */
        data[7] = 0xFF; /* Reserved */
        return CAN_Send_Message(ID, data);
    } else {
        /* Multiple messages - Load data */
        j1939->this_ecu_tp_cm.total_message_size_being_transmitted
            = (j1939->this_dm.errors_dm1_active * 4)
            + 2; /* set total message size where each DTC is 4 btyes, plus 2 bytes for the lamp code
                  */
        j1939->this_ecu_tp_cm.number_of_packages_beging_transmitted
            = (j1939->this_ecu_tp_cm.total_message_size_being_transmitted)
            / 7; /* set number of packages, where each package will transmit up to 7 bytes */
        if (j1939->this_ecu_tp_cm.total_message_size_being_transmitted % 7 > 0) {
            j1939->this_ecu_tp_cm
                .number_of_packages_beging_transmitted++; /* add extra frame if data rolls over */
        }

        /* Load lamp data to first two bytes */
        j1939->this_ecu_tp_dt.data[0]
            = (j1939->this_dm.dm1.SAE_lamp_status_malfunction_indicator << 6)
            | (j1939->this_dm.dm1.SAE_lamp_status_red_stop << 4)
            | (j1939->this_dm.dm1.SAE_lamp_status_amber_warning << 2)
            | (j1939->this_dm.dm1.SAE_lamp_status_protect_lamp);
        j1939->this_ecu_tp_dt.data[1]
            = (j1939->this_dm.dm1.SAE_flash_lamp_malfunction_indicator << 6)
            | (j1939->this_dm.dm1.SAE_flash_lamp_red_stop << 4)
            | (j1939->this_dm.dm1.SAE_flash_lamp_amber_warning << 2)
            | (j1939->this_dm.dm1.SAE_flash_lamp_protect_lamp);
        ...
        for (i = 0; i < j1939->this_ecu_tp_cm.total_message_size_being_transmitted; i++) {
            ...
            j1939->this_ecu_tp_dt.data[i * 4 + 4]
                = ((j1939->this_dm.dm1.SPN[i] >> 11) & 0b11100000) | j1939->this_dm.dm1.FMI[i];
            j1939->this_ecu_tp_dt.data[i * 4 + 5]
                = (j1939->this_dm.dm1.SPN_conversion_method[i] << 7)
                | j1939->this_dm.dm1.occurrence_count[i];
        }
 ...
 * PGN: 0x00FECA (65226)
 */
void SAE_J1939_Read_Response_Request_DM1(J1939_t* j1939,
                                         uint8_t SA,
                                         uint8_t data[],
                                         uint8_t errors_dm1_active)
{
    /* If there are fewer active DTCs than previous, clear the list so old DTCs do not remain after
     * parsing the new ones*/
    if (errors_dm1_active < j1939->from_other_ecu_dm.errors_dm1_active) {
        memset(&j1939->from_other_ecu_dm.dm1.SPN,
               0,
               sizeof(j1939->from_other_ecu_dm.dm1.SPN)); /* This set all fields of dm1 to 0 */
    }
    ...
    for (i = 0; i < errors_dm1_active && i < MAX_DM_FIELD; i++) {
        j1939->from_other_ecu_dm.dm1.SPN[i] = ((data[(i * 4) + 4] & 0b11100000) << 11)
            | (data[(i * 4) + 3] << 8) | data[(i * 4) + 2];
     ...
     * When there is a single DM1 code, and the SPN is 0, this signals all DM1 messages have cleared
