                                                    ...
                                                    uint16_t key)
{
    ...
    data[1]
        = ((number_of_requested_bytes >> 3) & 0xE0) | (pointer_type << 4) | (command << 1) | 0b1;
    data[2] = pointer;
    data[3] = pointer >> 8;
    data[4] = pointer >> 16;
...
ENUM_J1939_STATUS_CODES SAE_J1939_Read_Request_DM14(J1939_t* j1939, uint8_t DA, uint8_t data[])
{
    ...
    FLASH_EEPROM_RAM_Memory(&number_of_requested_bytes,
                            pointer_type,
                            &command,
                            &pointer,
                            &pointer_extension,
    ...
    uint16_t seed = key;
    status = SAE_J1939_Send_Response_DM15(j1939,
                                          DA,
                                          number_of_allowed_bytes,
                                          status,
                                          EDC_parameter,
                                          EDCP_extention,
                                          seed);
    ...
    if (status == STATUS_DM15_PROCEED) {
        if (SAE_J1939_Send_Binary_Data_Transfer_DM16(j1939,
                                                     DA,
                                                     number_of_allowed_bytes,
                                                     raw_binary_data)
            == STATUS_SEND_OK) {
            status = SAE_J1939_Send_Response_DM15(j1939,
                                                  DA,
                                                  number_of_allowed_bytes,
                                                  STATUS_DM15_OPERATION_COMPLETED,
                                                  EDC_parameter,
                                                  EDCP_extention,
                                                  seed);
        } else {
