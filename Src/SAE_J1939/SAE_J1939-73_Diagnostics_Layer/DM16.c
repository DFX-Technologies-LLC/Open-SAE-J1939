                                                                 ...
                                                                 uint8_t raw_binary_data[])
{
    if (number_of_occurences < 8) {
        ...
        for (i = 0; i < number_of_occurences; i++) {
            j1939->this_ecu_tp_dt.data[j1939->this_ecu_tp_cm.total_message_size_being_transmitted++]
                = raw_binary_data[i]; /* When i = 0, then total_message_size = 1 */
        }
        /* Send TP CM */
...
void SAE_J1939_Read_Binary_Data_Transfer_DM16(J1939_t* j1939, uint8_t SA, uint8_t data[])
{
    ...
    memset(j1939->from_other_ecu_dm.dm16.raw_binary_data,
           0,
           sizeof(j1939->from_other_ecu_dm.dm16.raw_binary_data));
    uint8_t i;
    for (i = 0; i < data[0]; i++) {
